...
export function Root() {
  ...
  // Se eliminó "Documentos"
  const { language } = useAccessibility();

  const navigation = [
    { nameKey: "nav.home", href: "/dashboard", icon: Home },
    { nameKey: "nav.faq", href: "/dashboard/preguntas-frecuentes", icon: HelpCircle },
    { nameKey: "nav.companies", href: "/dashboard/empresas", icon: Building2 },
    { nameKey: "nav.guide", href: "/dashboard/guia", icon: BookOpen },
    { nameKey: "nav.contacts", href: "/dashboard/contactos", icon: Users },
    { nameKey: "nav.accessibility", href: "/dashboard/accesibilidad", icon: Settings },
  ];
          ...
          <div className="mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-white text-lg sm:text-xl leading-tight">
                    {translate(language, "layout.appTitle")}
                  </h1>
                      ...
                      <div className="absolute right-0 mt-2 w-48 bg-card rounded-lg shadow-lg border border-border py-2 z-20">
                        <div className="px-4 py-3 border-b border-border">
                          <p className="text-sm font-medium text-foreground">
                            {translate(language, "layout.student")}
                          </p>
                        ...
                        >
                          <LogOut size={16} />
                          {translate(language, "common.logout")}
                        </button>
                      </div>
                    </>
                  )}
            ...
            <nav className="p-4 space-y-1">
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon size={20} />
                    <span>{translate(language, item.nameKey)}</span>
                  </Link>
                );
              })}
            </nav>
            ...
            <div className="lg:hidden fixed inset-0 top-16 bg-card z-40">
              <nav className="p-4 space-y-1">
                ...
                <div className="mb-4 p-4 bg-accent rounded-lg border border-primary/20">
                  ...
                  <button
                    onClick={handleLogout}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm text-destructive bg-card hover:bg-accent rounded-lg border border-destructive/20"
                  >
                    <LogOut size={16} />
                    {translate(language, "common.logout")}
                  </button>
